...
#sounds and music
sdeath=game.mixer.Sound('Sounds/death.wav')
sportal=game.mixer.Sound('Sounds/portal.wav')
skey=game.mixer.Sound('Sounds/key.wav')
select=game.mixer.Sound('Sounds/select.wav')
sgameover=game.mixer.Sound('Sounds/gameover.wav')
gamemusic='Sounds/music.mp3'
bossmusic='Sounds/boss.mp3'
titlemusic='Sounds/titlemusic.mp3'
...
#ENEMY ANIMATTION
def enemyAnimation():
    global enemyimgcount,eimg,eimg0,eimg1,eimg2,eimg3,eimg4
...
def enemyCollision(playerx,playery):
    global lives,xx,yy,enemycollision
...
#PORTAL ANIMATION
def portalAnimation():
    global portalimgcount,portalimg0,portalimg1,portalimg
    #check for image change
    portalimgcount+=1
    if portalimgcount==3:
...
#BOSS ANIMATION
def bossAnimation():
    global bossimgcount,bossimg0,bossimg1,bossimg
    #check for image change
    bossimgcount+=1
    if bossimgcount==3:
        bossimg=bossimg1
...
def portal(px,py,playerx,playery):
    global level,xx,yy,portalbox,reset,bosshp
    #draw
...
def keyLock(lx,ly,kx,ky):
    global unlocked,lockbox,keybox,collision,pbox
    #DRAW
    if unlocked==False:
...
#gameover check
def deathCheck():
    global lives,level,reset,lives,death
    if lives<=0:
        level=1
        lives=3
        game.time.wait(500)
...
#RESET VARIABLES
def resetVars(key):
    global enemyimgcount,enemyleft,enemyright,unlocked,collision,enemycollision,prevkey,enemies,walls,espawn,wspawn,bspawn,boss,bspd,bosshp,pleft,pright,px,py
    enemyimgcount=0
    enemyleft=False
    enemyright=True
    unlocked=key
